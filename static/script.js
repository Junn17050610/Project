...
function showResult(isRain, confidence, probabilities) {
  ...
  if (probabilities) {
    ...
    for (let [className, prob] of Object.entries(probabilities)) {
      ...
      // Jika backend memakai nama string "hujan" / "tidak_hujan"
      if (className.toLowerCase() === "hujan") {
        classLabel = "Hujan";
      } else if (className.toLowerCase() === "tidak_hujan") {
        classLabel = "Tidak Hujan";

        // Jika backend memakai angka 0 / 1
      } else if (className === "0") {
        classLabel = "Hujan";
      } else if (className === "1") {
        classLabel = "Tidak Hujan";
...
async function checkBackendConnection() {
  try {
    ...
    console.warn(
      "⚠ Backend not connected. Make sure Flask server is running at http://localhost:5000"
    );
  }
}
